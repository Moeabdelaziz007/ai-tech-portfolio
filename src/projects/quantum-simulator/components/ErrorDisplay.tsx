...
export const ErrorDisplay: React.FC<ErrorDisplayProps> = ({ errors, onClear }) => {
  if (errors.length === 0) {
    return null;
  }
    ...
    <div className="mb-6">
        ...
        <div className="flex items-center gap-2">
          {errorCount > 0 && (
            <div className="flex items-center gap-1 text-red-600">
              ...
              <span className="font-semibold">{errorCount} Error{errorCount !== 1 ? 's' : ''}</span>
            </div>
          )}
          {warningCount > 0 && (
            <div className="flex items-center gap-1 text-yellow-600">
              <FaExclamationTriangle size={16} />
              <span className="font-semibold">{warningCount} Warning{warningCount !== 1 ? 's' : ''}</span>
            </div>
          )}
        </div>
        {onClear && (
      ...
      {errorCount > 0 && (
        <div className="mt-3 p-2 bg-red-100 rounded text-sm text-red-700">
          <strong>Note:</strong> Please fix all errors before running the simulation.
        </div>
      )}
    </div>
  );
