...
const Chatbot = () => {
  ...
  const getBotResponse = (userMessage: string): string => {
    ...
    if (message.includes('مرحبا') || message.includes('hello') || message.includes('hi') || message.includes('أهلا')) {
      return botResponses.greetings[Math.floor(Math.random() * botResponses.greetings.length)];
    }
    
    if (message.includes('مشروع') || message.includes('project') || message.includes('عمل') || message.includes('أعمال')) {
      return botResponses.projects[Math.floor(Math.random() * botResponses.projects.length)];
    }
    
    if (message.includes('تواصل') || message.includes('contact') || message.includes('linkedin') || message.includes('github') || message.includes('ايميل')) {
      return botResponses.contact[Math.floor(Math.random() * botResponses.contact.length)];
    }
    
    if (message.includes('مهارة') || message.includes('skill') || message.includes('تقنية') || message.includes('technology') || message.includes('برمجة')) {
      return botResponses.skills[Math.floor(Math.random() * botResponses.skills.length)];
    }
    
    if (message.includes('أمريكي') || message.includes('amrikyy') || message.includes('سيرة') || message.includes('cv') || message.includes('من أنت')) {
      return botResponses.portfolio[Math.floor(Math.random() * botResponses.portfolio.length)];
    }
    
    if (message.includes('خبرة') || message.includes('experience') || message.includes('عملت') || message.includes('شركة')) {
      return botResponses.experience[Math.floor(Math.random() * botResponses.experience.length)];
    }
    
    if (message.includes('دراسة') || message.includes('جامعة') || message.includes('education') || message.includes('كوليدج')) {
      return botResponses.education[Math.floor(Math.random() * botResponses.education.length)];
    }
    
    if (message.includes('هدف') || message.includes('طموح') || message.includes('goal') || message.includes('مستقبل')) {
      return botResponses.goals[Math.floor(Math.random() * botResponses.goals.length)];
    }
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-neon-green/20">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <h3 className="text-tech-white font-semibold">أمريكي</h3>
